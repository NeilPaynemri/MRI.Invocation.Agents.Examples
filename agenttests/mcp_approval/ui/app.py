...
AGENT_NAME = "invoketest1-mcp-v7-custom-ce82"
        ...
        # ── Streaming path ────────────────────────────────────────
            ...
            try:
                ...
                for raw in event_stream:
                    ...
                    # ── custom (from get_stream_writer) ──
                    elif event_type == "custom":
                        status = event.get("status", "")
                        detail = event.get("detail", "")
                        custom_node = event.get("node", node)
                        if status in ("planning", "thinking", "executing_tools"):
                            thoughts.append(f"🔄 **{custom_node}**: {detail}")
                        elif status in ("plan_complete", "llm_done"):
                            thoughts.append(f"✅ **{custom_node}**: {detail}")
                        elif status == "tool_running":
                            thoughts.append(f"⚙️ {detail}")
                        elif status == "tool_result":
                            thoughts.append(f"&ensp;&ensp;📦 {detail}")
                        else:
                            thoughts.append(f"ℹ️ **{status}**: {detail}")
                        _render_thoughts()
                        status_area.info(f"⏳ {detail}")
