...
async def handle_invoke(request: Request) -> Response:
    ...
    logger.info(
        "POST /invocations  inv_id=%s  session=%s  thread=%s  user=%r  message=%r  approve=%s",
        invocation_id, session_id, thread_id, user_id, message[:80] if message else "", approve,
    )
    ...
    async def sse_stream():
        ...
        # Check if graph hit an interrupt (pending tool approval)
        state = await g.aget_state(config)
        if state.next:
            # Graph paused before tools node — emit approval_required
            last_msg = state.values["messages"][-1]
            pending_tool_calls = [
                {"id": tc["id"], "name": tc["name"], "args": tc["args"]}
                for tc in getattr(last_msg, "tool_calls", [])
            ]
            logger.info("Interrupt: awaiting approval for %s", [tc["name"] for tc in pending_tool_calls])
            yield f"data: {json.dumps({'event': 'approval_required', 'tool_calls': pending_tool_calls, 'thread_id': thread_id})}\n\n"
