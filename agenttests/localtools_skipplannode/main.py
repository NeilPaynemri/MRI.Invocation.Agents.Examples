...
logger = logging.getLogger("invoketest1")
        ...
        try:
            async for chunk in graph.astream(
                ...
                if chunk_type == "messages":
                    ...
                    # Skip plan_node messages — plan content stays in thoughts via custom events
            ...
            async for chunk in graph.astream(
                ...
                if chunk_type == "messages":
                    if isinstance(chunk_data, tuple) and len(chunk_data) == 2:
                        ...
                        # Skip plan_node messages in non-streaming too
                        if _meta.get("langgraph_node") == "plan_node":
                            continue
                        usage = getattr(msg, "usage_metadata", None)
                        if usage:
